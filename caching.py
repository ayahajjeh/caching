...
def request_byte(address, cache, last_used_address):
    ...
    print("D000000" + str(last_used_address) + ": addr " + hex(address) + "; looking for tag " + hex(address_tag) + " in set " + hex(address_set) + ".")
    print("\n")

    # print state of the set
    print("State of set " + hex(address_set) + ":")
    line_number = 0
    for L in cache.set[address_set]:
        print("line " + hex(line_number) + " V=" + str(int(L.valid)) + " tag " + hex(L.tag) + " last_touched=" + str(L.last_touched))
        line_number += 1
    print("\n")
    ...
    for L in set:
        if L.valid == True and L.tag == address_tag:
            ...
            print("Found it in line " + hex(line_number) + ".  Hit! Updating last_touch to " + str(last_used_address))
            break
        line_number += 1
    ...
    if not found_requested_byte:
        ...
        if line_index == -1:
            ...
            print("Miss! Evicting line " + hex(line_index) + "; adding block there; setting last_touch to " + str(last_used_address))
        else:
            # found am empty line
            print("Miss! Found empty line " + hex(line_index) + "; adding block there; setting last_touch to " + str(last_used_address))
...
def caching(cache, addresses):
    ...
    # address_number_so_far += 1
    # hits, misses = request_byte(addresses[0], cache, address_number_so_far)
    print_line()
    print("Hits: ", str(hits) + "; misses: " + str(misses) + "; addresses: " + str(address_number_so_far))

# please pass to the main function the path to the tiny trace text file
addresses = main("/Users/ayahajjeh/Desktop/23F/CS51/hw7/tiny-trace.txt")

# please comment out and uncomment any of the sections below to produce the output for the desired type of caching
# the following is the output for direct mapped cache
cache1 = cache("direct mapped")
caching(cache1, addresses)

# the following is the output for 2-way set associative cache
cache2 = cache("2-way set associative")
caching(cache2, addresses)

# the following is the output for 4-way set associative cache
cache3 = cache("4-way set associative")
caching(cache3, addresses)

# the following is the output for fully associative cache
